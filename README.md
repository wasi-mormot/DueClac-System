...
If you want the project to appear as a real Windows application in File Explorer, you need to build a separate `.exe`. A `.py` file itself will always stay a Python script.
...
```text
dist\DueClacSystem.exe
```

You can now double-click `DueClacSystem.exe` from File Explorer without opening `main.py` in Python.

Notes:

- Windows may show the file type as `Application` even if the `.exe` extension is hidden.
- If you want to see the `.exe` extension in File Explorer, enable `View -> Show -> File name extensions`.
- The build uses the included [DueClacSystem.spec](C:/Users/ASUS/Documents/Codex/2026-04-21-dueclac-system-pdf-to-excel-financial-2/DueClacSystem.spec) file.
